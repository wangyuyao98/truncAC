...
source("gen_HTE4.R")  # simulate T(a) from AFT model - the true tau() with nu = log() is a linear function in Z1
...
CATE.truth = cbind(1, dat$sinpiZ1, dat$Z2sqrt) %*% c(beta.T[2], beta.T[3], beta.T[4])
...
cov.names.T.oracle = c("A","A_sinpiZ1","A_Z2sqrt")
