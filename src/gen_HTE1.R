...
gen <- function(n, multi = 100, gamma, T.min, Q.min, D.min, Q.max, tau, 
    ...
    TT1 = T.min + rweibull(multi*n, shape = shape.T, scale = exp(-1/shape.T * (cbind(1, 1, Z1, Z2, Z1, Z2)%*%beta.T)))  # We include variables (1,a,a*Z1,Z2)
    TT0 = T.min + rweibull(multi*n, shape = shape.T, scale = exp(-1/shape.T * (cbind(1, 0, Z1, Z2, 0, 0)%*%beta.T)))
    TT = TT0
    TT[A==1] = TT1[A==1]
