...
gen <- function(n, multi = 100, gamma, T.min, Q.min, D.min, Q.max, tau, 
    ...
    Q2 = Q2.min + as.vector((Q2.max-Q2.min)*(1-U2^(exp(-cbind(1,A,Z1,Z2) %*% beta.Q))))
    QQ = tau - Q2
    
    TT1 = T.min + rweibull(multi*n, shape = shape.T, scale = exp(-1/shape.T * (cbind(1, 1, Z1, Z2)%*%beta.T)))
    TT0 = T.min + rweibull(multi*n, shape = shape.T, scale = exp(-1/shape.T * (cbind(1, 0, Z1, Z2)%*%beta.T)))
    TT = TT0
    TT[A==1] = TT1[A==1]
    
    D = D.min + rweibull(multi*n, shape = shape.C, scale = exp(-1/shape.C * (cbind(1, QQ, A, Z1, Z2)%*%beta.C)))
