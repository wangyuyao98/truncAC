# Generating data such that the CATE in terms of the survival function difference is a prespecified function


## Simulate T^*(a) from AFT model with log-normal error term

## simulate data with two covariates

gen <- function(n, multi = 100, gamma.A, T.min, Q.min, D.min, Q.max, tau, 
                beta.T, sigma_eps, bdd_eps,
                beta.Q, beta.C, shape.C){
    
    Z1 = runif(multi*n, -1, 1)
    Z2 = runif(multi*n, -1, 1)
    # Z3 = runif(multi*n, -1, 1)
    # Z4 = runif(multi*n, -1, 1)
    # Z5 = runif(multi*n, -1, 1)
    # Z6 = runif(multi*n, -1, 1)
    ...
    # Simulate event time from AFT model
    eps1 = pmax(pmin(rnorm(multi*n, mean = 0, sd = sigma_eps), bdd_eps), -bdd_eps)
    eps0 = pmax(pmin(rnorm(multi*n, mean = 0, sd = sigma_eps), bdd_eps), -bdd_eps)
    TT1 = exp(cbind(1,1,Z1,sqrt(abs(Z2))) %*% beta.T + eps1)
    TT0 = exp(cbind(1,0,0,sqrt(abs(Z2))) %*% beta.T + eps0)
    TT = TT0
    TT[A==1] = TT1[A==1]
    ...
    dat.full = data.frame(time.1 = TT1, time.0 = TT0, time = TT, C = C, 
                          Q = QQ, X = X, delta = delta, A = A, Z1 = Z1, Z2 = Z2)
    dat.all = dat.full
